...
- **Knowledge** (`/knowledge`) - RAG knowledge base list with rich cards (status, mode, categories, description, stats), create KB (with mode selector: manual/category, description guidance), delete with confirmation, chunk profiles table (name, size, overlap, splitter, tokenizer) with create/delete
- **KB Detail** (`/knowledge/:kbId`) - Full KB management: inline-editable name/description, stats cards (files, chunks, indexed, pending), category management (add/remove linked categories), bound files list with binding mode badges (pin/follow_latest), file removal, incremental/rebuild index actions
