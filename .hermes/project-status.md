...
- PR `#197` remote-feedback pass accepted all three Copilot comments as valid and addressed them in a follow-up fix: `Storage` now closes a partially opened connection if SQLite startup probing raises, the KB-list schema-object probe now uses the read-only/query-only SQLite path and fails closed for unreadable existing databases, and `Storage` startup schema preflight skips `PRAGMA quick_check` while explicit status/plan/apply keep it.
...
- Issue #196 PR `#197` remote-feedback validation: new regressions `python -m pytest tests/test_sqlite_schema_runner.py::test_storage_closes_connection_when_startup_probe_raises_sqlite_error tests/test_sqlite_schema_runner.py::test_storage_startup_status_skips_quick_check_but_schema_status_keeps_it tests/test_fastapi_rag_admin_endpoints.py::test_rag_admin_kb_list_user_schema_probe_is_read_only_query_only tests/test_fastapi_rag_admin_endpoints.py::test_rag_admin_kb_list_unreadable_sqlite_fails_closed_without_writes -q --tb=short --disable-warnings --no-cov` passed (`5 passed`); schema/storage focus `python -m pytest tests/test_sqlite_schema_runner.py tests/test_database_migration.py tests/test_production_recovery.py tests/test_storage_v2.py -q --tb=short --disable-warnings --no-cov` passed (`46 passed`); RAG admin KB-list coverage in `python -m pytest tests/test_fastapi_rag_admin_endpoints.py -q --tb=short --disable-warnings --no-cov` passed (`105 passed, 7 skipped`); touched-file `python -m ruff check ...` passed; `python -m compileall ai_actuarial tests` passed; `git diff --check`, `git diff --exit-code -- ai_actuarial/api/routers/rag_admin.py`, and `git diff --exit-code origin/main...HEAD -- ai_actuarial/api/routers/rag_admin.py` passed.
