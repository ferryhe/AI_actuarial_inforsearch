...
- Date: 2026-08-19
- Branch: `codex/issue-174-ready-data-chunk-content-events`
- Baseline: `origin/main` at `d9f0e0f2472dadbc01afd0a521fc3723b16f423e` (merged PR `#187`).
- Scope: Wire ready-data source-state events only to builder-effective canonical chunk-content changes through `Storage.replace_global_chunks()`, in the same SQLite transaction. Automatic execution/build/publish/GC, generic binding removal, Catalog/file/index/embedding events, UI, full-pipeline, deployment, production, and sibling repositories remain excluded.
- Publication is pending after the mandatory local gates. The intended PR will be Ready for review and use `Refs #174`, leaving Issue `#174` open.
...
- TDD evidence: the initial targeted collection was RED with `13 failed, 7 passed`. The two independent reviewers then identified a SQLite `TEXT`-affinity canonicalization gap; its regression test failed first and passed after the fix. The final content/binding/membership/builder/source-state/API/task/publication/GC suite passes `241 passed, 5 skipped`; skips are existing Windows link/reparse capability sentinels and the three SWIG deprecation warnings are pre-existing.
...
- Targeted Ruff, `python -m compileall -q ai_actuarial tests`, staged/untracked whitespace checks, and `git diff --check` pass. Independent specification and quality/security re-reviews are both CLEAN after the canonicalization fix.
- The mandatory local Codex CLI review could not start because Windows returned `Access is denied` for `C:\Program Files\WindowsApps\OpenAI.Codex_26.814.5167.0_x64__2p2nqsd0c76g0\app\resources\codex.exe` when invoked with `review --base origin/main`. It was not routed through an unknown or alternate entrypoint.
...
- Next action: publish the validated branch as a Ready-for-review PR, observe CI and remote/Copilot feedback for approximately 15 minutes, then hand it to the maintainer for review/merge while leaving Issue `#174` open.
