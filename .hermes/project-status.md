...
- Current Issue `#174` retention/GC files in scope are `.hermes/project-status.md`, `ai_actuarial/storage.py`, `ai_actuarial/api/services/rag_admin.py`, `tests/test_ready_data_retention_gc.py`, `tests/test_ready_data_publication.py`, and `tests/test_fastapi_rag_admin_endpoints.py`. `ai_actuarial/api/routers/rag_admin.py` retains its pre-existing line-ending-only worktree status with no content diff and is excluded from staging.
