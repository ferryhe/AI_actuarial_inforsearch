...
- `OPENAI_API_KEY`、`DEEPSEEK_API_KEY`、`MISTRAL_API_KEY`、`SILICONFLOW_API_KEY`、`OPENROUTER_API_KEY`、`DASHSCOPE_API_KEY`、`MOONSHOT_API_KEY`、`KIMI_API_KEY`、`ZHIPUAI_API_KEY`、`ANTHROPIC_API_KEY`、`GOOGLE_API_KEY`、`GOOGLE_CLOUD_API_KEY`、`COHERE_API_KEY`、`MINIMAX_API_KEY`、`HUGGINGFACE_API_KEY`：可选 AI/转换 key；支持模型列表 API 的 provider 也会用这些环境变量做在线发现。
