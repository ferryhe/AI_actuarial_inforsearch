...
本分支为 **纯 Flask 归档分支**，仅保留 Flask 模板界面与 Flask API，作为 React/FastAPI 迁移前的稳定回退基线。

| 界面 | 技术栈 | 访问方式 | 特点 |
|------|--------|----------|------|
| **Flask 模板界面** | Python + Jinja2 HTML | `http://localhost:8000` | 服务端渲染，功能完整，部署简单 |

- 页面由 Python 直接渲染，无需额外前端构建服务。
- 所有业务 API 仍由同一个 Flask 应用提供。
- 该分支的目标是保留一套可独立运行的 Flask 版本，供归档与回退使用。
...
│   └─ web/                # Flask web app and API (app.py, chat_routes.py, rag_routes.py)
