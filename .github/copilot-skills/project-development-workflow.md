...
- ⚠️ **Remind user**: "Phase 0已完成，建议创建PR进行第一次审查。" ("Phase 0 completed, suggest creating PR for initial review.")
...
4. **Incremental Commits** (REQUIRED - Not Just Mental Checkpoints)




   - Make ACTUAL git commits frequently, not just mental notes
   ...
   - Use clear commit messages describing what was accomplished




   - Mark task as complete in todo list after committing
   ...
   **Commit Frequency Guidelines**:
   - Minimum: 1 commit per phase
   - Recommended: 3-5 commits per phase
...
- ⚠️ **Remind user**: "Phase 1核心实现已完成，建议提交PR进行代码审查。" ("Phase 1 core implementation completed, suggest submitting PR for code review.")
...
- ⚠️ **Remind user**: "Phase 2自动化测试已完成，建议创建PR记录测试结果。" ("Phase 2 automated testing completed, suggest creating PR to document test results.")
...
**Phase 3 Completion**:
- Commit the testing guide
- ⚠️ **Remind user**: "Phase 3测试准备已完成，建议提交PR以便审查测试计划。" ("Phase 3 test preparation completed, suggest submitting PR to review test plan.")
...
**Phase 4 Completion**:
- Commit any bug fixes made during user testing
- ⚠️ **Remind user**: "Phase 4用户测试已通过，建议创建PR记录测试结果和修复内容。" ("Phase 4 user testing passed, suggest creating PR to document test results and fixes.")
...
1. **Documentation Location and Naming Convention**

   **REQUIRED**: All documentation must follow this structure:
   
   - **Location**: Root directory `docs/` folder
   - **Naming Format**: `文档类型-YYYY-MM-DD-阶段描述.md`
   
   **Examples**:
   ```
   docs/开发日志-2026-02-10-阶段一第六步实现.md
   docs/实现报告-2026-02-10-文件选择器功能完成.md
   docs/测试指南-2026-02-09-用户测试准备.md
   docs/技术方案-2026-02-08-Phase0规划.md
   ```
   
   **Document Types** (文档类型):
   - `开发日志` - Development Log (daily progress)
   - `实现报告` - Implementation Report (final summary)
   - `测试指南` - Testing Guide (for user testing)
   - `技术方案` - Technical Design (architecture/design)
   - `规划文档` - Planning Document (Phase 0 plan)
   
   **Each Development Log Must Include**:
   - Progress summary for the day/session
   - Completed tasks
   - Technical decisions made
   - **🧪 用户测试清单 (User Testing Checklist)** - List of items user needs to test
...
**Deliverable**: Documentation in `docs/` folder following naming convention

**Example**: `docs/实现报告-2026-02-10-文件选择器功能完成.md`

**Phase 5 Completion**:
- Commit all documentation
- ⚠️ **Remind user**: "Phase 5文档已完成，建议创建PR以便审查最终文档。" ("Phase 5 documentation completed, suggest creating PR to review final documentation.")
...
- ⚠️ **IMPORTANT - Remind user**: "🎉 所有阶段已完成！请创建最终PR以便合并到主分支。" ("🎉 All phases completed! Please create final PR to merge into main branch.")
- Suggest PR title and description format
