...
class QueryRouter:
    ...
    def select_kb(
        ...
        for kb in available_kbs:
            score = self._calculate_relevance(kb, keywords, entities, categories)
            kb_scores[kb.kb_id] = {"score": score, "name": kb.name}

        # Sort by score
        sorted_kbs = sorted(kb_scores.items(), key=lambda x: x[1]["score"], reverse=True)
        ...
        for kb_id, info in sorted_kbs:
            if info["score"] >= threshold:
                ...
                logger.info(f"Selected KB '{info['name']}' (id={kb_id}, score={info['score']:.2f})")
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Query analysis: intent={intent}, complexity={complexity}, " f"categories={categories}"
        )
    ...
    def _calculate_relevance(
        self, kb: KnowledgeBase, keywords: List[str], entities: List[str], categories: Set[str]
    ) -> float:
        """
