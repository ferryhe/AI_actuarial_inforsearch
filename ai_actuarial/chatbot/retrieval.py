...
class RAGRetriever:
    ...
    def __init__(self, storage: Storage, config: Optional[ChatbotConfig] = None):
        """
    ...
    def get_current_embedding_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "provider": str(self.embedding_generator.provider or "openai").strip().lower()
            or "openai",
            "model": str(
                self.embedding_generator.config.embedding_model or "text-embedding-3-large"
            ).strip()
    ...
    def retrieve(
        ...
        try:
            ...
            if len(kb_ids) == 1:
                ...
                results = self._retrieve_from_multiple_kbs(kb_ids, query_vector, top_k, threshold)

            if not results:
                raise NoResultsException(f"No results found above similarity threshold {threshold}")
    ...
    def _retrieve_from_kb(
        ...
        # Search
        search_results = vector_store.search(query_vector, k=top_k, similarity_threshold=threshold)
    ...
    def _retrieve_from_multiple_kbs(
        self, kb_ids: List[str], query_vector: np.ndarray, top_k: int, threshold: float
    ) -> List[Dict[str, Any]]:
        """
        ...
        balanced_results = self._ensure_kb_diversity(
            ranked_results, all_results, min_per_kb=self.config.min_results_per_kb
        )

        return balanced_results[:top_k]

    def _deduplicate_chunks(
        self, chunks: List[Dict[str, Any]], similarity_threshold: float = 0.95
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _rank_with_diversity(
        self, chunks: List[Dict[str, Any]], kb_ids: List[str]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _ensure_kb_diversity(
        ...
        # Re-sort by score
        balanced_chunks.sort(key=lambda x: x["metadata"]["similarity_score"], reverse=True)
    ...
    def _get_vector_store(self, kb_id: str) -> VectorStore:
        ...
        if not index_path.exists():
            raise RetrievalException(f"Index file not found for KB '{kb_id}': {index_path}")
        ...
        if dimension in (None, ""):
            dimension = getattr(kb, "embedding_dimension", None) or infer_embedding_dimension(
                kb.embedding_model
            )
        if dimension in (None, ""):
            raise RetrievalException(f"Unable to determine index dimension for KB '{kb_id}'")

        vector_store = VectorStore(dimension=int(dimension), index_path=str(index_path))
    ...
    def _ensure_kb_embedding_compatibility(
        ...
        if index_dimension in (None, ""):
            index_dimension = getattr(kb, "embedding_dimension", None) or infer_embedding_dimension(
                index_model
            )
        ...
        if (
            ...
            and str(index_provider).strip().lower() != current_provider
        ):
            mismatch = True
        ...
        if mismatch:
            raise EmbeddingConfigurationMismatchException(
                ...
                current_dimension=(
                    int(current_dimension) if current_dimension not in (None, "") else None
                ),
