...
def create_storage_from_config(
    config: dict[str, Any],
) -> Union[Storage, "StorageV2", "StorageV2Full"]:
    ...
    # Legacy mode: use paths.db
        ...
        raise ValueError("Invalid configuration: must provide either 'database' or 'paths.db'")
...
def get_database_config_from_env() -> dict[str, Any]:
    ...
    if db_type == "sqlite":
        config["database"] = {"type": "sqlite", "path": os.getenv("DB_PATH", "data/index.db")}
    elif db_type == "postgresql":
        config["database"] = {
            "type": "postgresql",
