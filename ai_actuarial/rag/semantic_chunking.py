...
class SemanticChunker:
                    ...
                    if section_text:
                        ...
                        if token_count > self.max_tokens:
                            ...
                            for sub_chunk in sub_chunks:
                                ...
                                sub_chunk.section_hierarchy = (
                                    " > ".join(current_hierarchy) if current_hierarchy else None
                                )
    ...
    def _chunk_by_paragraphs(self, content: str, metadata: Dict[str, Any]) -> List[Chunk]:
        ...
        if current_chunk_text:
            ...
            if (
                token_count >= self.min_tokens or not chunks
            ):  # Include if meets min or is only chunk
                chunks.append(
                    Chunk(
                        content=chunk_text,
                        token_count=token_count,
    ...
    def _chunk_by_sentences(
        self, text: str, metadata: Dict[str, Any], start_index: int = 0
    ) -> List[Chunk]:
        """
    ...
    def _are_chunks_valid(self, chunks: List[Chunk]) -> bool:
        ...
        valid_count = sum(
            1 for chunk in chunks if self.min_tokens <= chunk.token_count <= self.max_tokens
        )
