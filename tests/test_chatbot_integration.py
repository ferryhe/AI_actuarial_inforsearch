...
class TestChatbotIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = ChatbotConfig(api_key="test-key-12345", top_k=3, similarity_threshold=0.5)
    ...
    def test_retrieval_single_kb(self, mock_vector_store):
        ...
        self.assertEqual(
            results[0]["content"], "Capital requirements are essential for insurance companies."
        )
    ...
    def test_retrieval_multi_kb(self, mock_vector_store):
        ...
        results = self.retriever.retrieve(
            query="insurance requirements", kb_ids=["test_kb1", "test_kb2"], top_k=5
        )
    ...
    def test_retrieval_no_results(self, mock_vector_store):
        ...
        with self.assertRaises(NoResultsException):
            self.retriever.retrieve(query="obscure query with no matches", kb_ids="test_kb1")
    ...
    def test_retrieval_invalid_kb(self, mock_vector_store):
        ...
        with self.assertRaises(InvalidKBException):
            self.retriever.retrieve(query="test query", kb_ids="nonexistent_kb")
    ...
    def test_llm_generation_success(self, mock_openai_class):
        ...
        mock_response.choices[0].message.content = (
            "This is a test response with proper citations [Source: test.pdf]."
        )
    ...
    @patch("openai.OpenAI")
    def test_llm_generation_keeps_max_tokens_for_openai_compatible_providers(
    ...
    def test_llm_generation_retry_on_rate_limit(self, mock_openai_class):
        ...
        config = ChatbotConfig(
            api_key="test-key", max_retries=3, retry_delay=0.1, exponential_backoff=False
        )
        llm_client = LLMClient(config)
    ...
    def test_llm_generation_max_retries_exceeded(self, mock_openai_class):
        ...
        mock_client.chat.completions.create.side_effect = APITimeoutError(
            request=_mock_openai_request()
        )

        mock_openai_class.return_value = mock_client

        # Create LLM client with fast retry
        config = ChatbotConfig(api_key="test-key", max_retries=2, retry_delay=0.1)
        llm_client = LLMClient(config)
    ...
    def test_full_query_workflow(self, mock_openai_class, mock_vector_store):
        ...
        conv_id = self.conv_manager.create_conversation(
            user_id=user_id, kb_id="test_kb1", mode="expert"
        )
        ...
        self.conv_manager.add_message(conv_id, "assistant", response, citations=citations)
    ...
    def test_multi_turn_conversation(self, mock_openai_class, mock_vector_store):
        ...
        # Create conversation
        conv_id = self.conv_manager.create_conversation(user_id="test_user", mode="expert")
        ...
        mock_response1.choices[0].message.content = (
            "Solvency II is a regulatory framework [Source: scr_guide.pdf]."
        )
        ...
        mock_response2.choices[0].message.content = (
            "The SCR uses standard formula or internal model [Source: scr_guide.pdf]."
        )
    ...
    def test_error_handling_retrieval_failure(self, mock_vector_store):
        ...
        self.retriever.embedding_generator.generate_single.side_effect = Exception(
            "Embedding failed"
        )

        # Should raise RetrievalException
        with self.assertRaises(RetrievalException):
            self.retriever.retrieve(query="test query", kb_ids="test_kb1")
