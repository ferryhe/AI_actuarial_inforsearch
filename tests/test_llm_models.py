...
class TestModelCache:
    ...
    def test_get_models_all_providers(self):
        ...
        with (
            patch.object(cache, "_fetch_openai_models", return_value=DEFAULT_MODELS["openai"]),
            patch.object(cache, "_fetch_mistral_models", return_value=DEFAULT_MODELS["mistral"]),
            patch.object(
                cache, "_fetch_siliconflow_models", return_value=DEFAULT_MODELS["siliconflow"]
            ),
            patch.object(
                cache,
                "_fetch_openai_compatible_models",
                side_effect=lambda provider, **kwargs: DEFAULT_MODELS.get(provider, []),
            ),
        ):
...
class TestDefaultModels:
    ...
    def test_recommended_markdown_tools_are_available_as_ocr_models(self):
        local_ocr_names = {
            model["name"] for model in DEFAULT_MODELS["local"] if "ocr" in model["types"]
        }
        mathpix_ocr_names = {
            model["name"] for model in DEFAULT_MODELS["mathpix"] if "ocr" in model["types"]
        }
    ...
    def test_build_models_from_ids_falls_back_for_unknown_modalities(self):
        models = _build_models_from_ids(
            "openai", {"omni-moderation-latest"}, DEFAULT_MODELS["openai"]
        )
        assert models == DEFAULT_MODELS["openai"]
