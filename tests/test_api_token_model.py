...
class TestApiToken:
    ...
    def test_default_values(self, session):
        ...
        token = ApiToken(
            provider="brave", category="search", api_key_encrypted="encrypted_brave_key"
        )
    ...
    def test_unique_provider_category_constraint(self, session):
        ...
        token1 = ApiToken(provider="openai", category="llm", api_key_encrypted="key1")
        session.add(token1)
        session.commit()

        token2 = ApiToken(provider="openai", category="llm", api_key_encrypted="key2")
        session.add(token2)
    ...
    def test_different_category_allows_duplicate_provider(self, session):
        ...
        token1 = ApiToken(provider="openai", category="llm", api_key_encrypted="key1")
        token2 = ApiToken(provider="openai", category="embeddings", api_key_encrypted="key2")
    ...
    def test_to_dict_timestamps_serialization(self, session):
        ...
        token = ApiToken(
            provider="serpapi", category="search", api_key_encrypted="encrypted_serpapi_key"
        )
    ...
    def test_repr_string(self, session):
        ...
        token = ApiToken(provider="openai", category="llm", api_key_encrypted="encrypted_key")
    ...
    def test_update_timestamps(self, session):
        ...
        token = ApiToken(provider="siliconflow", category="llm", api_key_encrypted="encrypted_key")
    ...
    def test_optional_fields(self, session):
        ...
        token = ApiToken(
            provider="test_provider", category="test", api_key_encrypted="encrypted_key"
        )
    ...
    def test_query_by_provider_and_category(self, session):
        ...
        token1 = ApiToken(provider="openai", category="llm", api_key_encrypted="key1")
        token2 = ApiToken(provider="brave", category="search", api_key_encrypted="key2")
        token3 = ApiToken(provider="openai", category="embeddings", api_key_encrypted="key3")
        ...
        # Query by both
        specific_token = (
            session.query(ApiToken).filter_by(provider="openai", category="llm").first()
        )
    ...
    def test_status_field(self, session):
        ...
        active_token = ApiToken(
            provider="provider1", category="llm", api_key_encrypted="key1", status="active"
        )
        inactive_token = ApiToken(
            provider="provider2", category="llm", api_key_encrypted="key2", status="inactive"
        )
